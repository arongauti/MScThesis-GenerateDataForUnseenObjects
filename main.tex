...
\newML{AbstractText}
{%english
The objective of this project was to use a robot to generate training data for previously unseen objects. By examining certain aspects, whether it is possible to generate new arrangements of object using a robot manipulator, to annotate objects automatically by determining the extent of the objects in the images, and to improve the performance of a Convolutional Neural Network using automatically generated training data from an robot.
Data were collected from both former student and by the author himself by having the robot pick up objects and move them repeatedly.
The results of this project showed that the robot is capable of moving and rotating objects around inside the bin to create new arrangements. The automatically annotated images shows an good annotation almost every time, which save time and effort. 
\textbf{Á eftir að skrifa meira}
% The results shows that automatically annotating images would save time and effort. It shows an good annotation in  annotating images is not easy for human and takes time. The results shows that annotating images from the robot would save time 

%  of objects are created and then it is possible to automatically annotate them
}
% Icelandic abstract goes here
{
Markmið verkefnisins var að nota vélmenni til að búa til þjálfunargögn fyrir áður óséða hluti. Með því að skoða ákveðna þætti, þar á meðal hvort mögulegt væri að ??? og að bæta afköst  ????.
Gögnum var safnað frá bæði fyrrum nemenda við Háskólann í Reykjavík og af höfundinum sjálfum með því að láta vélmennið taka upp hluti og færa þá ítrekað/endurtekið.
...
\include{text/introduction}%%RUM: Introduction
\include{text/methods}%%RUM: "Methods"
%\part{The Second Part}
\include{text/results}%%RUM: "Results"
\include{text/discussion}%%RUM: "Discussion"
...
% Tók þetta út\input{code} % as an example, perhaps some of your code
